  ...
  async function resolveSessionForPayment(args, extra) {
    ...
    if (!session) {
      return {
        session: null,
        sessionHint: null,
        sessionResolution: { mode: 'missing', reason: 'no_bound_session' },
        error: {
          status: 402,
          mode: 'session_required',
          error: 'session_missing',
          message: 'No active OpenDexter session is bound to this conversation.',
          hint: 'Call x402_wallet() to create or resume a session before paying.',
          nextAction: 'call_x402_wallet',
        },
      };
    }
